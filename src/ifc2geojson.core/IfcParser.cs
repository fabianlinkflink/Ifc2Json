...
namespace ifc2geojson.core
{
    public static class IfcParser
    {
        ...
        public static Project ParseModel(IfcStore model)
        {
            ...
            if (ifcWalls.Count == 0)
            {
                List<IIfcBuildingElementProxy> ifcWalls2 = new List<IIfcBuildingElementProxy>();

                var ifcWall = model.FederatedInstances.OfType<IIfcBuildingElementProxy>().ToList();
                foreach (var element in ifcWall)
                {
                    var prop = GetPropertiesFromObject(element);
                    if (prop.ContainsValue("Wall"))
                        ifcWalls2.Add(element);
                }

                var walls = ParseWalls(ifcWalls2);
                project.Walls = walls;
            }
            else
            {
        ...
        private static List<Wall> ParseWalls(List<IIfcBuildingElementProxy> ifcWalls)
        {
            var walls = new List<Wall>();
            foreach (var ifcWall in ifcWalls)
            {
        ...
        private static void ParseRepresentation(IIfcProductRepresentation ifcProductRepresentation, Element element)
        {
            if (ifcProductRepresentation != null)
            {
                ...
                /* We are not using the bounding boxes anymore
